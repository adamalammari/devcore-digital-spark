...
function findAnswer(input: string): string {
  ...
  const keywords: Record<string, string[]> = {
    ...
    "كم سعر الموقع؟": ["سعر", "موقع", "تعريفي", "شركة", "كم"],
    ...
    "هل يمكن التعاون معكم؟": ["تعاون", "عمولة", "شراكة", "مسوق"],
    "متى تُدفع العمولة؟": ["دفع", "عمولة", "متى"],
    "كيف أتواصل معكم؟": ["تواصل", "واتساب", "رقم", "هاتف"],
    "أريد بدء مشروع": ["بدء", "مشروع", "أريد", "أبدأ", "طلب"],
  };
...
const Chatbot = ({ isOpen, onClose }: ChatbotProps) => {
  ...
  const handleSend = (text: string) => {
    ...
    setInput("");
    // Move to next page of questions
  ...
  return (
    <div className="fixed bottom-4 left-4 right-4 md:left-auto md:right-6 md:bottom-6 z-50 md:w-[400px]">
      <div className="card-elevated rounded-2xl overflow-hidden shadow-2xl border border-border flex flex-col" style={{ maxHeight: "85vh" }}>
        {/* Header */}
        <div className="gradient-bg p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <img src={logo} alt="DevCore" className="h-8 w-auto" />
            <div>
              <h3 className="text-primary-foreground font-heading font-bold text-sm">مساعد DevCore</h3>
            ...
            <div key={i} className={`flex ${m.role === "user" ? "justify-start" : "justify-end"}`}>
              <div
                className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm whitespace-pre-wrap ${
                  m.role === "user"
                    ? "gradient-bg text-primary-foreground rounded-tl-sm"
        ...
        <a
          ...
          className="mx-4 mb-2 flex items-center justify-center gap-2 bg-green-500 text-white py-2 rounded-xl text-sm font-semibold hover:bg-green-600 transition-colors"
        >
        ...
        <div className="p-3 border-t border-border bg-background flex gap-2">
          <input
            ...
            className="flex-1 bg-secondary rounded-xl px-4 py-2 text-sm text-foreground placeholder:text-muted-foreground outline-none focus:ring-2 focus:ring-primary/30"
          />
          <button
            onClick={() => handleSend(input)}
            className="gradient-bg text-primary-foreground p-2 rounded-xl hover:opacity-90 transition-opacity"
          >
            <Send size={18} />
          </button>
        </div>
...
export const ChatbotButton = ({ onClick }: { onClick: () => void }) => (
  <button
    onClick={onClick}
    className="fixed bottom-6 left-6 z-40 gradient-bg text-primary-foreground w-14 h-14 rounded-full flex items-center justify-center shadow-xl animate-pulse-glow hover:opacity-90 transition-opacity"
  >
